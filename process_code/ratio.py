...
def process_data(input_file_path, output_file_path):
    ...
    grouped_data = weekly_data.groupby(['Facility Name', 'Week Number'])


    positive_count = grouped_data['Sentiment Numeric'].apply(lambda x: (x == 1).sum()).rename('Positive_Count')
    negative_count = grouped_data['Sentiment Numeric'].apply(lambda x: (x == 0).sum()).rename('Negative_Count')


    overall_count = weekly_data.groupby('Week Number')['Sentiment Numeric'].count().rename('Overall Count')
    prison_count = grouped_data['Sentiment Numeric'].count().rename('Prison Count')


    merged_data = pd.concat([positive_count, negative_count, prison_count], axis=1).reset_index()
    merged_data = merged_data.merge(overall_count, on='Week Number')
    ...
    final_table = merged_data[['Facility Name', 'Week Number', 'Positive_Count', 'Negative_Count', 
